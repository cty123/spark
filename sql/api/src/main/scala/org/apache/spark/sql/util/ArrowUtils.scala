...
private[sql] object ArrowUtils {
  ...
  def toArrowType(dt: DataType, timeZoneId: String, largeVarTypes: Boolean = false): ArrowType =
    dt match {
      ...
      case DecimalType.Fixed(precision, scale) if precision <= DecimalType.DECIMAL_128_PRECISION =>
        new ArrowType.Decimal(precision, scale, 8 * 16)
      case DecimalType.Fixed(precision, scale) => new ArrowType.Decimal(precision, scale, 8 * 32)
